...
import { coreItems, bonusItems, upsellItems, PRICES } from "@/data/pricing";
...
const META: Record<string, { label: string; blurb: string; icon: string }> = {
  "Functional Website (15 to 20 pages)": { label: "Functional Website", blurb: "Up to 20 pages, loads in 2 seconds, sells and converts.", icon: "Monitor" },
  ...
  "Missed Call Text Back": { label: "Missed Call Text Back", blurb: "4 channels covered: forms, live chat, calls and social.", icon: "MessageSquare" },
  "Automated Lead Follow-Up": { label: "Automated Lead Follow-Up", blurb: "Three follow-ups per lead, three reminders per job.", icon: "Repeat" },
  "5-Star Review Funnel": { label: "5-Star Review Funnel", blurb: "Auto-asks happy customers the moment jobs wrap.", icon: "Star" },
  "More Lead Marketing Campaigns": { label: "More Lead Marketing Campaigns", blurb: "Old lead revival and past customer review campaigns.", icon: "Megaphone" },
  "Maps Multiplier": { label: "Maps Multiplier", blurb: "Apple Maps, Bing, Yelp, Facebook, BBB and Chamber of Commerce, all built out.", icon: "Map" },
  "Citation Cascade": { label: "Citation Cascade", blurb: "Listed on YellowPages, Thumbtack, HomeAdvisor, Nextdoor and more.", icon: "List" },
  "Pipeline Pilot": { label: "Pipeline Pilot", blurb: "Leads move through the right stages automatically.", icon: "GitBranch" },
  "Review Responder": { label: "Review Responder", blurb: "Professional, personalized reply to every Google review.", icon: "MessageCircle" },
  "Profile Poster": { label: "Profile Poster", blurb: "2 to 3 posts a week to your Google Business Profile.", icon: "Send" },
  "AI Chat & Calendar Booking Agent": { label: "AI Chat & Calendar Booking Agent", blurb: "Replies the second a lead messages and books the job into your calendar, day or night.", icon: "CalendarCheck" },
};
...
const ICON_PATHS: Record<string, string> = {
  ...
  CalendarCheck: "M8 2v4 M16 2v4 M3 10h18 M21 8a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v11a2 2 0 0 0 2 2h6 M16 19l2 2 4-4",
};
...
interface Item { title: string; body?: string; lead?: string; bullets?: string[]; }

function Row({ item, monthly, open, onToggle, value }: { item: Item; monthly: boolean; open: boolean; onToggle: () => void; value?: string }) {
  ...
  const detail = item.lead && item.bullets
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {value && (
                  <span className="inline-block rounded-full bg-red text-white text-[10.5px] font-bold tracking-[0.05em] px-[8px] py-[2px] leading-none whitespace-nowrap">
                    {value}
                  </span>
                )}
              </div>
            ...
            <button
              ...
              className={`ml-1 flex-none inline-flex items-center justify-center w-6 h-6 rounded-full border transition-colors duration-200 cursor-pointer ${open ? "bg-navy border-navy text-white" : "bg-transparent border-slate/30 text-slate/60 hover:text-navy hover:border-navy/40"}`}
            >
      ...
      {open && (
        <tr>
          <td colSpan={3} className="pb-4 pt-0">
            <div id={detailId} className="ff-detail-in rounded-xl bg-ivory border border-hairline px-4 py-3 text-[13.5px] leading-[1.6] text-slate whitespace-pre-line">
              {detail}
            </div>
          </td>
        </tr>
...
export default function FeatureMatrix() {
  ...
  const bookingAgent = upsellItems.find((u) => u.title === "AI Chat & Calendar Booking Agent");
      ...
      <div className="max-w-[920px] mx-auto px-[clamp(20px,4vw,64px)] py-[clamp(48px,6vw,88px)]">
        <Reveal className="bg-white border border-hairline rounded-[20px] shadow-[0_20px_44px_rgba(11,16,32,0.06)] px-[clamp(16px,3vw,36px)] pb-[clamp(20px,3vw,32px)] pt-2 overflow-x-auto">
          <table className="w-full border-collapse min-w-0">
            <colgroup>
              <col />
              <col className="w-[54px] sm:w-[132px]" />
              <col className="w-[54px] sm:w-[132px]" />
            </colgroup>
            <thead>
              <tr>
                <th className="sticky top-0 z-10 bg-white text-left pt-6 pb-4 align-bottom">
                  ...
                  <span className="block mt-1 text-[12.5px] font-normal text-slate normal-case tracking-normal">Tap the info icon on any line for the full detail.</span>
                </th>
                ...
                <th className="sticky top-0 z-10 bg-red/[0.06] rounded-t-xl pt-6 pb-4 text-center align-bottom">
                  <span className="inline-block mb-1 rounded-full bg-red text-white px-2 sm:px-2.5 py-0.5 text-[9px] sm:text-[10px] font-bold tracking-[0.04em] sm:tracking-[0.06em] uppercase whitespace-nowrap"><span className="sm:hidden">Best</span><span className="hidden sm:inline">Best value</span></span>
            ...
            <tbody>
              ...
              <BandRow label="Annual bonus stack · yearly only · $4,985 free" />
              {bonusItems.map((item) => (
                <Row key={item.title} item={item} monthly={false} open={openKey === item.title} onToggle={() => toggle(item.title)} value={item.value} />
              ))}
              {bookingAgent && (
                <>
                  <BandRow label="Also free on the yearly plan" />
                  <Row key={bookingAgent.title} item={bookingAgent} monthly={false} open={openKey === bookingAgent.title} onToggle={() => toggle(bookingAgent.title)} value="$2,000/mo value" />
                </>
              )}
            </tbody>
          </table>
